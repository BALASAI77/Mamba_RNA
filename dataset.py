...
class RNADataset(Dataset):
    def __init__(self, csv_file, tokenizer=None, max_length=512):
        # Detect header
        try:
            # Read first line to check for header
            df_head = pd.read_csv(csv_file, nrows=5)
            # If 'seq' is in columns, it likely has a header
            if 'seq' in df_head.columns or 'name' in df_head.columns:
                 self.data = pd.read_csv(csv_file)
            else:
                 # No header, assign names
                 self.data = pd.read_csv(csv_file, header=None, names=['id', 'seq', 'structure'])
        except Exception:
             # Fallback
             self.data = pd.read_csv(csv_file)
